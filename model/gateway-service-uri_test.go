...
func TestURIParser_cleanHosts(t *testing.T) {
	...
	result := Gateway_NewURIParser().Gateway_CleanHosts(samples)
	for i := range result {
...
func TestURIParser_cleanPath(t *testing.T) {
	...
	subject := Gateway_NewURIParser()

	for i := range samples {
		if have := subject.Gateway_CleanPath(samples[i]); expected[i] != have {
			t.Errorf("want: %s, have: %s\n", expected[i], have)
		}
	}
}
...
func TestURIParser_getEndpointPath(t *testing.T) {
	...
	sc := Gateway_ServiceConfig{}
	subject := Gateway_NewURIParser()

	for i := range samples {
		params := sc.extractPlaceHoldersFromURLTemplate(samples[i], endpointURLKeysPattern)
		if have := subject.Gateway_GetEndpointPath(samples[i], params); expected[i] != have {
			t.Errorf("want: %s, have: %s\n", expected[i], have)
		}
	}
}