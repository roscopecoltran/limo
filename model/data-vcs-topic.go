...
type Topic struct {
	...
	//sorting.SortingDESC
	Name  string `gorm:"type:varchar(64);not null" json:"name" yaml:"name"`
	Count int    `gorm:"-" json:"count" yaml:"count"`
	Stars []Star `gorm:"many2many:star_topics;"  json:"-" yaml:"-"`
}
...
func TestTopicsGraph(query string) (taggraph.Tagger, error) {
	...
	if !ok {
		log.WithFields(
			logrus.Fields{"action": "PrintTopicsGraph",
				"step":  "GetTag",
				"model": "Topic",
				"query": query}).Warnf("Tag topic not found", query)
