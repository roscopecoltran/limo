...
func CountPublicationDateByHoursAgo(hoursAgoStr string) string {
	...
	if err != nil {
		log.WithError(err).WithFields(
			logrus.Fields{	
				"prefix": 			"util-time",
				"src.file": 		"utils/helper.go",
				"method.name": 		"CountPublicationDateByHoursAgo", 
				"mthod.prev": 		"time.LoadLocation(...)", 
				}).Fatal("could not load the location.")
				panic(err)
	}
	estTime 		:= t.In(est)
	...
	if err != nil {
		log.WithError(err).WithFields(
			logrus.Fields{	
				"prefix": 			"util-time",
				"src.file": 		"utils/helper.go",
				"method.name": 		"CountPublicationDateByMinutesAgo", 
				"mthod.prev": 		"time.LoadLocation(...)", 
				}).Fatal("could not load the location.")
				panic(err)
	}
